...
def process_gtd_inv_spec(source_path, save_path, valid_folders):
    ...
    def get_gtd_sort_key_from_file(fname):
        """
        Ключ сортировки для GTD: сначала дата (средний блок ДДММГГ), затем порядковый номер (последний блок).
        Пример: GTD_10137010_300426_5011010 → дата 30.04.26, номер 5011010.
        ...
        if len(parts) >= 2 and parts[-1].isdigit() and parts[-2].isdigit():
            ...
            if date_tuple is not None:
                return (0, date_tuple, reg)
        for part in reversed(parts):
            if part.isdigit():
                return (1, (9999, 99, 99), int(part))
        return (1, (9999, 99, 99), float("inf"))
